...
```
data/
└── rag_text/               # Arquivos de texto já limpos e padronizados
    ├── 01_LEI_N_11.947_DE_16_DE_JUNHO_DE_2009_Lei_da_Alimentao_Escolar_2009.txt
    ├── 02_PORTARIA_MDA_N_49_DE_16_DE_OUTURO_DE_2024_Institui_o_Primeiro_Plano_Nacional_de_.txt
    └── 019_LEI_N_14.935_DE_26_DE_JULHO_DE_2024__Lei.pdf.txt
```

**Importante:** Os arquivos devem ser arquivos `.txt` já limpos e processados por scripts de pré-processamento.

## Passo a Passo

### 1. Preparar arquivos de texto limpos

Os arquivos de texto devem estar já limpos e padronizados em `data/rag_text/`:

```bash
# Criar diretório (se não existir)
mkdir -p data/rag_text

# Adicionar arquivos .txt já processados/limpos
# (arquivos devem ter sido pré-processados por scripts de limpeza)
cp /caminho/para/seu/documento.txt data/rag_text/
```
...
O script irá:
1. Ler todos os arquivos `.txt` de `data/rag_text/`
...
```bash
python scripts/ingest_rag_documents.py --manual
```
...
3. Salvar como `.txt` em `data/rag_text/`
...
cp seus_documentos_limpos.txt data/rag_text/
...
- Verifique se os arquivos estão em `data/rag_text/`
...
- Verifique se perguntas de onboarding estão populadas (`python seeds_onboarding.py`)
